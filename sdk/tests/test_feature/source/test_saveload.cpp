...
//		engine->SetEngineProperty(asEP_BUILD_WITHOUT_LINE_CUES, 1);
	...
	// Reported by Julius Narvilas/MrFloat
	{
		...
		if (bout.buffer != " (0, 0) : Error   : Shared type 'Test1' doesn't match the original declaration in other module\n"
						   " (0, 0) : Error   : LoadByteCode failed. The bytecode is invalid. Number of bytes read from stream: 166\n"
	...
	// http://www.gamedev.net/topic/664405-scripthelper-config-helpers-not-working-correctly/
	{
		...
		if( bout.buffer != "config (61, 0) : Warning : Cannot register template callback without the actual implementation\n" )
		{
			PRINTF("%s", bout.buffer.c_str());
			TEST_FAILED;
		}
...
//		engine->RegisterObjectMethod("typeof<T>", "type opImplConv()", asFUNCTION(0), asCALL_GENERIC);
		...
		if( bout.buffer != " (0, 0) : Error   : Template type 'typeof' doesn't exist\n"
						   " (0, 0) : Error   : LoadByteCode failed. The bytecode is invalid. Number of bytes read from stream: 235\n" )
		{
			PRINTF("%s", bout.buffer.c_str());
			TEST_FAILED;
		}
