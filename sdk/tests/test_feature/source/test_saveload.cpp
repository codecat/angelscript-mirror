...
bool Test()
{
	...
	// http://www.gamedev.net/topic/664405-scripthelper-config-helpers-not-working-correctly/
	{
		...
		if( bout.buffer != "config (54, 0) : Warning : Cannot register template callback without the actual implementation\n" )
		{
			PRINTF("%s", bout.buffer.c_str());
			TEST_FAILED;
		}
...
//		engine->RegisterObjectMethod("typeof<T>", "type opImplConv()", asFUNCTION(0), asCALL_GENERIC);
		...
		if( bout.buffer != " (0, 0) : Error   : Template type 'typeof' doesn't exist\n"
						   " (0, 0) : Error   : LoadByteCode failed. The bytecode is invalid. Number of bytes read from stream: 122\n" )
		{
			PRINTF("%s", bout.buffer.c_str());
			TEST_FAILED;
		}
