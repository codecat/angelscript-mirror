...
void RegisterBStr(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("bstr", "uint length()", asFUNCTION(asBStrLengthMethod), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	r = engine->RegisterObjectBehaviour("bstr", asBEHAVE_ASSIGNMENT, "bstr &f(bstr &in)", asFUNCTION(asBStrCopy),      asCALL_CDECL_OBJLAST); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("bstr", asBEHAVE_ADD_ASSIGN, "bstr &f(bstr &in)", asFUNCTION(asBStrAppend),    asCALL_CDECL_OBJLAST); assert( r >= 0 );
